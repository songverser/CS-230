...
Hit Mesh::Intersection(const Ray& ray, int part) const
{
    ...
    ans.part = -1;
    double dist;
    if(Intersect_Triangle(ray,part,dist)==true){ 
        if(dist>small_t && dist < ans.dist){
            ans.dist = dist;
            ans.part = part;
        }
    }
...
bool Mesh::Intersect_Triangle(const Ray& ray, int tri, double& dist) const
{
    ...
    if(gamma>=0 && s>=0 && beta>=0 && 1-beta-gamma>=0) // inside triangle
    {
        dist = s;
        return true;
    }
...
Box Mesh::Bounding_Box(int part) const
{
    ...
    maxxyz.fill(-1000000);
    for(int i=0;i<3;i++){
        vec3 pt = vertices[triangles[part][i]];
